...
params = pd.DataFrame(list(zip(feature.columns, model.coef_[0])), columns=["Factor", "Weight"]).sort_values("Weight", ascending=False)
params = params.append({"Factor":"Intercept", "Weight": model.intercept_[0]}, ignore_index=True)
params.to_csv(model_path.replace("pickle", "csv"), index=0)